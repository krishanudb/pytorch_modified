...
def pad_to_square(img, pad_value, mask = None):
    c, h, w = img.shape
    dim_diff = np.abs(h - w)
    ...
    if mask:
        mask = F.pad(img, pad, "constant", value=0)
        return img, mask, pad
    else:
        return img, pad

def resize(image, size, mask = None):
    image = F.interpolate(image.unsqueeze(0), size=size, mode="nearest").squeeze(0)
    if mask:
        mask = F.interpolate(image.unsqueeze(0), size=size, mode="nearest").squeeze(0)
        return image, mask
    else:
        return image


def random_resize(images, masks = None, min_size=288, max_size=448):
...
class ListDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        mask_path = self.mask_files[index % len(self.mask_files)].rstrip()
        # Extract image as PyTorch tensor
        img = transforms.ToTensor()(Image.open(img_path).convert('RGB'))
        mask = transforms.ToTensor()(Image.open(mask_path).convert('RGB'))
